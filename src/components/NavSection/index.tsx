...
function NavItem({ item }: { item: NavItemProps }) {
  ...
  const isHasActiveSub = children
    ? children.some(
        (item) =>
          item.path && !!matchPath({ path: item.path, end: false }, pathname)
      )
    : false;
  const isActiveRoot =
    (path && !!matchPath({ path, end: false }, pathname)) || isHasActiveSub;
  ...
  return (
    ...
    >
      <ListItemButton
        ...
        onClick={() =>
          title === "Theme" ? onClick && onClick() : handleNavigation(item.path)
        }
      >
...
export default function NavSection({
  ...
  const handleOpen =
    (children: any) => (event: React.MouseEvent<HTMLElement>) => {
